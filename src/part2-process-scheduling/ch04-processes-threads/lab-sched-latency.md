...
- Apply a mitigation (nice, affinity, or cgroup) and verify the tail
  improves
- Write down a *prediction* before each measurement and *rule out*
  one alternative explanation in your final report
...
Write this in your report **before** running the probe, using only
the Chapter 4 reading:

> *"On my idle machine, my baseline p99 wakeup latency will be
> approximately ____ µs, because the dominant source of delay on an
> idle CPU is ____."*

Fill in both blanks. The number is a single value (not a range),
and the reason should name a specific mechanism (timer-tick
granularity, idle-state exit latency, scheduler tick rate). A
prediction of "small" without a number is not a prediction.

After you measure, your report must (a) record the actual number
and (b) say whether you were within 2×, off by more than 2× but in
the right direction, or wrong about the dominant mechanism.
...
Following the same template as A.0:

> *"With 4 hogs pinned to CPU 0 alongside the probe, p99 wakeup
> latency will grow by approximately ____× relative to baseline,
> because each wakeup will now wait behind ____ runnable tasks of
> ____ ms quantum each."*
...
1. **`report.md`** — the narrative. For each part: prediction,
   numbers, mechanism. Include a table showing p50/p90/p99 across
   baseline / contended / mitigated, and a paragraph reflecting on
   whether your A.0 and B.0 predictions matched.
2. **`baseline.csv`**, **`contended.csv`**, **`mitigated.csv`** —
   raw percentile data, generated by your own runs (not by AI).
...
5. **Ruled-out alternative** — one paragraph naming a different
   plausible mechanism for your contended-run p99 inflation
   (cache pollution, NUMA migration, throttling, hypervisor
   preemption, IRQ steering, or another candidate of your choice)
   and citing one signal in your data that *excludes* it. "My
   `cpu-migrations` count from `perf stat` was 12 in both runs, so
   migration cannot explain the 100× p99 inflation" is the form
   of the argument.
...
| Criterion | Points |
|---|---|
| Pre-run predictions (A.0 and B.0) written before measurements | 10 |
| Correct baseline + contended percentiles, with enough samples (≥10 000) | 25 |
| Supporting OS signal collected and interpreted | 20 |
| One mitigation applied; before/after comparison clear | 20 |
| Mechanism explanation ties numbers to runqueue delay | 15 |
| Ruled-out alternative with supporting signal | 10 |

**Total: 100 points.**

Note that points 1, 5, and 6 are the AI-resistant components: an LLM
can invent prose for the mechanism paragraph, but it cannot
generate a plausible *prediction tied to your specific machine* or a
*ruled-out alternative grounded in your raw counters*. Honest
disclosure of AI use is graded in the Evidence Trail; non-disclosure
or fabricated raw data is treated as academic dishonesty.
